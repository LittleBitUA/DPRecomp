...
#include "deadlyprem_acquire_wizard.h"
...
std::filesystem::path PickIsoFile() {
  ...
  ofn.lpstrTitle = L"Select your Deadly Premonition Xbox 360 disc image";
...
std::filesystem::path PickIsoFile() {
  GtkWidget* dialog = gtk_file_chooser_dialog_new(
      "Select your Deadly Premonition Xbox 360 disc image", nullptr,
...
// Restart the process after the install so the fresh launch sees the game
// data (resuming the runtime inline hangs on Win32). Ported from the Downpour
// title update installer (RelaunchSelfOrResume).
// Region pick after an install (2026-09-06): the PAL and USA discs ship
// different executables, so the release carries two recompiled builds. Choose
// the one matching the freshly extracted default.xex by size (PAL 10,113,024;
// USA 10,080,256) and relaunch it with our own command line; unknown sizes keep
// the current executable.
static std::filesystem::path PickExecutableForGameRoot(const std::filesystem::path& game_root) {
#if defined(_WIN32)
  ...
  std::error_code ec;
  const auto size = std::filesystem::file_size(game_root / "default.xex", ec);
  if (ec) {
    return self;
  }
  const char* wanted = nullptr;
  if (size == 10080256ull) {
    wanted = "deadlyprem_usa.exe";
  } else if (size == 10113024ull) {
    wanted = "deadlyprem.exe";
  }
  if (wanted) {
    std::filesystem::path candidate = self.parent_path() / wanted;
...
#if defined(_WIN32)
  const std::filesystem::path exe = PickExecutableForGameRoot(runtime_paths.game_data_root);
  if (!exe.empty()) {
    // Reuse our command line but swap the program path (argv[0]) for the
    // regional build; any --flags survive.
    std::wstring cmd = GetCommandLineW();
    std::wstring rest;
    if (!cmd.empty() && cmd[0] == L'"') {
      size_t close = cmd.find(L'"', 1);
      rest = close == std::wstring::npos ? L"" : cmd.substr(close + 1);
    } else {
      size_t space = cmd.find(L' ');
      rest = space == std::wstring::npos ? L"" : cmd.substr(space);
    }
    std::wstring new_cmd = L"\"" + exe.wstring() + L"\"" + rest;
    std::vector<wchar_t> cmd_buf(new_cmd.begin(), new_cmd.end());
    cmd_buf.push_back(L'\0');
    STARTUPINFOW si{};
    si.cb = sizeof(si);
    PROCESS_INFORMATION pi{};
    if (CreateProcessW(exe.c_str(), cmd_buf.data(), nullptr, nullptr, FALSE, 0, nullptr, nullptr,
                       &si, &pi)) {
      CloseHandle(pi.hThread);
      CloseHandle(pi.hProcess);
      REXLOG_INFO("Game data installed; starting {} to pick up the files.", exe.filename().string());
      ExitProcess(0);
    }
    REXLOG_ERROR("Failed to start {} after install (Win32 error {}); resuming inline.",
                 exe.filename().string(), GetLastError());
  }
#endif
  if (complete) {
...
void ShowIsoInstallWizard(rex::ui::ImGuiDrawer* drawer, rex::PathConfig runtime_paths,
  ...
  AcquireWizardDialog::Options options;
  // ASCII only: the overlay font atlas has no em-dash glyph (renders '?').
  options.title = "Deadly Premonition - Game Data";
  options.intro =
      "This port needs the game files from your own legally-owned Xbox 360 copy "
      "of Deadly Premonition. Select your disc image (.iso) and its contents "
      "will be extracted here - nothing else to do.";
  options.target_directory = game_root.string();
  options.initial_status =
      "Select the disc image dumped from your copy of the game (Europe/PAL "
      "release; the USA disc is not supported yet). Extraction needs ~5.5 GiB "
      "of free space.";
