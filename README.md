...
> [!IMPORTANT]
> **1.3 (September 2026): the sky is fixed.** The console's HDR format keeps 2 bits of alpha and the game draws its sky and fog masks into them; 1.3 keeps a 16-bit shadow alpha next to the EDRAM buffer, so the stair-stepped sky, the hard tree crowns and the glass edges of the ROV path are gone. Also: FSR 3 no longer flickers between frames (it was auto-exposing every frame), 60 FPS runs on whole ticks like the console (the fix for #12, and for scripted scenes running fast), plus an experimental switch for the doubled sounds. Details in [What's new in 1.3](#whats-new-in-13). **Testers wanted** - see the notes.
>
...
>
> **1.2 (September 2026): keyboard and PlayStation button prompts.** The game's A / B / X / Y / LB / RB / LT / RT icons are replaced by pictures of *your* keys, generated by the launcher from your key bindings, or by PlayStation glyphs for DualShock / DualSense players — see [Keyboard prompts and texture modding](#keyboard-prompts-and-texture-modding). The same mechanism lets anyone swap any texture in the game with a PNG. Also: tap-to-shake QTEs, map zoom with the wheel, Alt+Tab, the 60 FPS tick follow-up (#12). Details in [What's new in 1.2](#whats-new-in-12). **1.2.1** fixes the black screen with FSR 2 / FSR 3 when Steam's Game Recording is on (#13).
>
...
- [Keyboard prompts and texture modding](#keyboard-prompts-and-texture-modding)
...
Full notes: [docs/RELEASE-NOTES-1.3.0.md](docs/RELEASE-NOTES-1.3.0.md).

![Sky, tree crowns and car glass after the fix (ROV path, 2x)](docs/screenshots/sky_fix.jpg)
...
- **Doubled sounds / prop swaps at 60 FPS (#17, #18)**: the animation event dispatcher is mapped; an experimental `dp_anim_event_dedupe` switch and a `dp_anim_event_log` are in, off by default - see the notes for what we still need from testers.
...
**1.2.1 (hotfix):** the FSR 2 / FSR 3 presenter pass left the wrong descriptor heap bound for the sharpening pass (D3D12 error #708 every frame). GPUs let it slide, Steam's background Game Recording did not: black screen with sound (#13). Fixed and verified under the D3D12 debug layer. Notes: [docs/RELEASE-NOTES-1.2.1.md](docs/RELEASE-NOTES-1.2.1.md).

Full 1.2 notes: [docs/RELEASE-NOTES-1.2.0.md](docs/RELEASE-NOTES-1.2.0.md).
...
- **60 FPS (#12 follow-up)**: the tick can go below 1.0 (`dp_60fps_tick_min`, default 0.5), so the game no longer runs ahead of the audio when the limiter releases frames early.
...
- Steam **Game Recording** + FSR 2 / FSR 3 gave a black screen in 1.2 (issue #13); fixed in 1.2.1. If you still see one, send `logs\deadlyprem_XXX.log`.
